...
def download_sample_for_info_vector(info_vector):
    ...
    # if image_id % 1 == 0:
    if image_id % 100 == 0:
        completion = 100.0-(100.0*image_id/3227131)
        print("[{}] downloading id {} of 3227131 = {:.4f}%".format(timestamp(), image_id, completion))
        sys.stdout.flush()
...
print("checking all files downloaded so far...")
sys.stdout.flush()
...
with open("res/tags_all/all_gelbooru.xml", "r") as all_info_vectors:
    ...
    from tqdm import *
    print("skipping images that have already been downloaded...")
    sys.stdout.flush()
    for i in tqdm(xrange(starting_index)):
...
#             m = re.search(r'\sid="(\d+)"\s', info_vector)
#             image_id = int(m.group(1)) if m else -1
#             if image_id == last_id_downloaded:
...
#             else:
#                 # if image_id % 1000 == 0:
#                 #     print("skipping already downloaded file {}".format(image_id))
#                 #     sys.stdout.flush()
#                 continue
